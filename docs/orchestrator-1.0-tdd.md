Orchestrator 1.0 — Technical Design Document
1. Overview
1.1 Purpose

Orchestrator 1.0 is a GitHub-native automation system that manages:
...
A GitHub App (Orchestrator) +

A set of reusable GitHub Actions workflows +

Config files & telemetry conventions within each repository.

This document defines the technical design for Orchestrator 1.0, focused on single-repo operation (multi-repo comes later with Codex SaaS).

1.2 Goals

Provide a repeatable, installable “Orchestrator” bundle any repo can adopt.
...
Repo-Level Workflows (Core of Orchestrator 1.0)
...
Call Orchestrator’s logic (containerized or composite actions).
...
For Orchestrator 1.0, all persistent state is GitHub objects + JSON files in the repo.
...
Quick signal whether Orchestrator is correctly configured.
...
Any optional Orchestrator-specific tokens (v2+).
...
Orchestrator logic should be:

Implemented in a single reusable action (e.g., orchestrator-core).
...
Maintain a dedicated orchestrator-test-repo:
...
Create Orchestrator config file schema and add JSON Schema validation.

Implement orchestrator-core action (Node/TypeScript or Python):
